...
# Import smc from the local feature_engineering_computations module
from .feature_engineering_computations.smc import smc
...
def generate_trading_gif(symbol="BTCUSDT", start_str="2025-07-28", timeframe="15m", window=100, limit=500):
    """
    Generate a trading GIF with SMC analysis
    
    Args:
        symbol (str): Trading symbol (default: "BTCUSDT")
        start_str (str): Start date for data (default: "2024-04-01")
        timeframe (str): Timeframe for data (default: "15m")
        window (int): Window size for rolling analysis (default: 100)
        limit (int): Limit of candles to process (default: 500)
    
    Returns:
        BytesIO: GIF data as BytesIO object
    """
    # Import data
    df = import_data(symbol, start_str, timeframe)
    df = df.iloc[-limit:]
    ...
    # Create GIF in memory instead of saving to file
    gif_buffer = BytesIO()
    imageio.mimsave(gif_buffer, gif, format='GIF', duration=1)
    gif_buffer.seek(0)
